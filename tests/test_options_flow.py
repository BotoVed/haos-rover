...
class TestAsyncStepNetwork:
    """Network settings: tcp_port, local_ip, ssid."""
    ...
    async def test_creates_entry_with_user_input(self, flow) -> None:
        user_input = {"tcp_port": 9999, "local_ip": "10.0.0.5", "ssid": "IoT_Net"}
        result = await flow.async_step_network(user_input)
        assert result["type"] == "create_entry"
        assert result["title"] == ""
        assert result["data"] == user_input

    async def test_accepts_min_tcp_port(self, flow) -> None:
        user_input = {"tcp_port": 1024, "local_ip": "", "ssid": ""}
        result = await flow.async_step_network(user_input)
        assert result["type"] == "create_entry"

    async def test_accepts_max_tcp_port(self, flow) -> None:
        user_input = {"tcp_port": 65535, "local_ip": "", "ssid": ""}
        result = await flow.async_step_network(user_input)
        assert result["type"] == "create_entry"

    async def test_accepts_empty_ip_and_ssid(self, flow) -> None:
        user_input = {"tcp_port": 4242, "local_ip": "", "ssid": ""}
        result = await flow.async_step_network(user_input)
        assert result["type"] == "create_entry"

    async def test_does_not_abort_when_runtime_is_none(self, flow) -> None:
        """network step does NOT check runtime/registry."""
        flow.config_entry.runtime_data = None
        result = await flow.async_step_network()
        assert result["type"] == "form"
        assert result["step_id"] == "network"
...
# =========================================================================
# Tests for async_step_pending
# =========================================================================


class TestAsyncStepPending:
    """Approve or reject pending registration requests."""

    async def test_shows_form_with_pending_options(
        self, flow, mock_registry
    ) -> None:
    ...
    async def test_aborts_when_no_pending(self, flow, mock_registry) -> None:
        mock_registry.all_pending = MagicMock(return_value=[])
        result = await flow.async_step_pending()
        assert result["type"] == "abort"
        assert result["reason"] == "no_pending"

    async def test_approves_pending_and_returns_to_menu(
        self, flow, mock_registry
    ) -> None:
        user_input = {"pending_hash": "cc0033 — Charlie", "action": "approve"}
        result = await flow.async_step_pending(user_input)
        mock_registry.approve_pending.assert_awaited_once_with("cc0033")
        mock_registry.revoke_user.assert_not_called()
        assert result["type"] == "menu"

    async def test_revokes_pending_and_returns_to_menu(
        self, flow, mock_registry
    ) -> None:
        user_input = {"pending_hash": "dd0044 — Diana", "action": "revoke"}
        result = await flow.async_step_pending(user_input)
        mock_registry.deny_pending.assert_awaited_once_with("dd0044")
        mock_registry.approve_pending.assert_not_called()
        assert result["type"] == "menu"

    async def test_defaults_to_approve_when_action_not_provided(
        self, flow, mock_registry
    ) -> None:
        """If action key is missing, the else branch calls deny_pending."""
        user_input = {"pending_hash": "cc0033 — Charlie"}
        result = await flow.async_step_pending(user_input)
        mock_registry.approve_pending.assert_not_called()
        mock_registry.deny_pending.assert_awaited_once_with("cc0033")
        assert result["type"] == "menu"

    async def test_empty_hash_skips_action(
        self, flow, mock_registry
    ) -> None:
        user_input = {"pending_hash": "", "action": "approve"}
        result = await flow.async_step_pending(user_input)
        mock_registry.approve_pending.assert_not_called()
        mock_registry.revoke_user.assert_not_called()
        assert result["type"] == "menu"
...
# =========================================================================
# Tests for async_step_config
# =========================================================================


class TestAsyncStepConfig:
    """Configuration export — QR code and hashes."""

    async def test_shows_form_with_description_placeholders(
        self, flow, mock_registry
    ) -> None:
        ...
        # QR image
        assert placeholders["qr"].startswith("![QR](")
        assert "api.qrserver.com" in placeholders["qr"]
        assert "qr-code" in placeholders["qr"]
        # Identity
        ...
        assert placeholders["hash_a"] == "2345"
        assert placeholders["hash_d"] == "6789"

    async def test_returns_to_menu_when_user_input_provided(
        self, flow
    ) -> None:
        result = await flow.async_step_config({})
        assert result["type"] == "menu"

    async def test_uses_unknown_for_missing_identity_hash(
        self, flow, mock_runtime
    ) -> None:
    ...
    async def test_uses_unknown_for_empty_identity_hash(
        self, flow, mock_runtime
    ) -> None:
    ...
    async def test_qr_url_contains_encoded_payload(
        self, flow
    ) -> None:
        ...
        # The payload should be URL-encoded in the data parameter
...
# =========================================================================
# Tests — parametrized abort for all runtime/registry-sensitive steps
# =========================================================================


class TestAbortWhenNotLoaded:
    """Parametrized: every step that guards runtime/registry should abort."""
    ...
    async def test_all_steps_abort_when_runtime_is_none(
        self, flow, step_name
    ) -> None:
        flow.config_entry.runtime_data = None
        method = getattr(flow, f"async_step_{step_name}")
        result = await method()
        assert result["type"] == "abort", (
            f"async_step_{step_name} did not abort when runtime is None"
        )
        assert result["reason"] == "not_loaded"
    ...
    async def test_all_steps_abort_when_registry_is_none(
        self, flow, step_name
    ) -> None:
        flow.config_entry.runtime_data.registry = None
        method = getattr(flow, f"async_step_{step_name}")
        result = await method()
        assert result["type"] == "abort", (
            f"async_step_{step_name} did not abort when registry is None"
        )
        assert result["reason"] == "not_loaded"
    ...
    async def test_steps_that_do_not_check_runtime(
        self, flow, step_name
    ) -> None:
        """These steps don't guard runtime/registry — they should NOT abort."""
        flow.config_entry.runtime_data = None
        method = getattr(flow, f"async_step_{step_name}")
        result = await method()
        assert result["type"] != "abort", (
            f"async_step_{step_name} unexpectedly aborted with reason={result.get('reason')}"
        )


# =========================================================================
# Tests for strings.json completeness
# =========================================================================


class TestStringsJson:
    """Verify strings.json has entries for all step_ids and abort reasons."""

    STRINGS_PATH = "custom_components/rover/strings.json"

    def test_all_step_ids_have_strings(self) -> None:
        """Every step_id referenced in options_flow.py must have a strings.json entry."""
        import pathlib
        import json as _json

        path = pathlib.Path(self.STRINGS_PATH)
        assert path.exists(), f"{self.STRINGS_PATH} not found"

        strings = _json.loads(path.read_text(encoding="utf-8"))
        option_steps = strings.get("options", {}).get("step", {})

        expected_steps = {
            "init", "general", "network", "devices",
            "remove_device", "test_device", "test_action",
            "users", "pending", "config",
        }

        for step_id in expected_steps:
            assert step_id in option_steps, (
                f"strings.json missing 'options.step.{step_id}'"
            )

    def test_all_abort_reasons_have_strings(self) -> None:
        """Every abort reason must have a strings.json entry."""
        import pathlib
        import json as _json

        path = pathlib.Path(self.STRINGS_PATH)
        strings = _json.loads(path.read_text(encoding="utf-8"))
        abort_reasons = strings.get("options", {}).get("abort", {})

        expected_reasons = {"not_loaded", "no_devices", "no_users", "no_pending"}
        for reason in expected_reasons:
            assert reason in abort_reasons, (
                f"strings.json missing 'options.abort.{reason}'"
            )
